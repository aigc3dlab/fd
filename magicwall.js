class MagicWall {
    ...
    drawPanel(ctx, items) {
        ...
        if (this.gameType === 'color' || this.gameType === 'dynamic') {
        ...
        } else if (this.gameType === 'mahjong' || this.gameType === 'emoji' || this.gameType === 'emoji2') {
            // 麻将墙 / Emoji墙 - 彩色背景方块上叠加字符
            const bgPalettes = {
                mahjong: [
                    '#c0392b', // 红
                    '#2980b9', // 蓝
            ...
            // 建立字符 → 颜色的映射（固定映射，保证两边一致性）
            const paletteKey = this.gameType === 'emoji2' ? 'emoji2' : this.gameType;
            const palette = bgPalettes[paletteKey] || bgPalettes['emoji'];
